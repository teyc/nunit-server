...
{
    $status = "<form class=`"inline`" method=`"POST`" action=`"./$currentSession.ps1`"><input type=submit value=`"Rerun all tests in current session`"></form>"
}
...
<p>
    <form class="inline" action="./new-session.ps1"><input type="submit" value="Re-run failed tests" /></form>
    $status
</p>
